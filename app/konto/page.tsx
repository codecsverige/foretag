          ...
          {activeTab === 'bookings' && <>
            ...
            {filtered.length > 0 ? filtered.map(b => { const s = statusConfig(b.status); return (
              <div key={b.id} className="bg-white rounded-2xl border overflow-hidden">
                <div className="p-5 flex gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${s.bg}`}><s.icon className={`w-6 h-6 ${s.text}`} /></div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2"><h3 className="font-semibold">{b.customerName || 'Kund'}</h3><span className={`px-2 py-0.5 text-xs rounded-full ${s.bg} ${s.text}`}>{s.label}</span></div>
                    <p className="text-gray-600">{b.service || b.serviceName}</p>
                    <div className="flex gap-4 mt-2 text-sm text-gray-500">
                      <span><HiOutlineCalendar className="w-4 h-4 inline mr-1" />{b.date}</span>
                      <span><HiOutlineClock className="w-4 h-4 inline mr-1" />{b.time}</span>
                      {b.customerPhone && <a href={`tel:${b.customerPhone}`} className="text-brand"><HiOutlinePhone className="w-4 h-4 inline mr-1" />{b.customerPhone}</a>}
                    </div>
                  </div>
                  {b.servicePrice && <p className="text-xl font-bold text-brand">{b.servicePrice} kr</p>}
                </div>
                {b.status === 'pending' && <div className="px-5 py-3 bg-yellow-50 border-t flex gap-3">
                  <button onClick={() => handleBooking(b.id, 'confirmed')} disabled={updating === b.id} className="flex-1 py-2 bg-green-500 text-white rounded-xl font-medium"><HiCheck className="w-5 h-5 inline mr-1" />Bekräfta</button>
                  <button onClick={() => handleBooking(b.id, 'cancelled')} disabled={updating === b.id} className="flex-1 py-2 border border-red-200 text-red-600 rounded-xl font-medium"><HiX className="w-5 h-5 inline mr-1" />Avböj</button>
                </div>}
                {b.status === 'confirmed' && <div className="px-5 py-3 bg-green-50 border-t">
                  <button onClick={() => handleBooking(b.id, 'completed')} disabled={updating === b.id} className="w-full py-2 bg-blue-500 text-white rounded-xl font-medium"><HiOutlineCheckCircle className="w-5 h-5 inline mr-1" />Markera slutförd</button>
                </div>}
              </div>
