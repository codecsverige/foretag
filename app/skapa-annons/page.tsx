...
import { HiPlus, HiTrash, HiCheck, HiExclamationCircle, HiPhotograph } from 'react-icons/hi'
...
export default function CreateAdPage() {
  ...
  const uploadImages = async (): Promise<string[]> => {
    if (images.length === 0 || !storage) return []
    ...
    try {
      for (const image of images) {
        ...
        const fileName = `ads/${timestamp}_${image.name}`
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          ...
          {step === 1 && (
            <div className="space-y-6">
              ...
              <button
                onClick={() => setStep(2)}
                disabled={!companyName || !category || !city || !description || !phone}
          ...
          {step === 2 && (
            <div className="space-y-6">
              <h2 className="text-xl font-bold text-gray-900">💈 Tjänster & Bilder</h2>
          ...
          {step === 3 && (
            <div className="space-y-6">
              <h2 className="text-xl font-bold text-gray-900">✅ Granska & publicera</h2>

              <div className="space-y-4">
                <div className="p-4 bg-gray-50 rounded-xl">
                  <h3 className="font-semibold text-gray-900 mb-2">Företagsinformation</h3>
                  <p className="text-sm text-gray-600">Företag: {companyName}</p>
                  <p className="text-sm text-gray-600">Kategori: {categories.find(c => c.id === category)?.name}</p>
                  <p className="text-sm text-gray-600">Stad: {city}</p>
                </div>

                <div className="p-4 bg-gray-50 rounded-xl">
                  <h3 className="font-semibold text-gray-900 mb-2">Tjänster</h3>
                  <p className="text-sm text-gray-600">{services.filter(s => s.name && s.price).length} tjänster</p>
                </div>

                {images.length > 0 && (
                  <div className="p-4 bg-gray-50 rounded-xl">
                    <h3 className="font-semibold text-gray-900 mb-2">Bilder</h3>
                    <p className="text-sm text-gray-600">{images.length} bild(er)</p>
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-3">
                ...
                >
                  {isSubmitting ? (uploadingImages ? 'Laddar upp bilder...' : 'Publicerar...') : '🚀 Publicera annons'}
                </button>
              </div>
            </div>
          )}
