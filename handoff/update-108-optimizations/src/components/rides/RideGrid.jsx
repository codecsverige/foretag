...
export default function RideGrid({ filters, onRidesChange, directRideId, onSearchChange }) {
  // Build server-side conditions for scalable querying
  const conditions = useMemo(() => {
    const c = [];
    // Filter by normalized origin/destination when provided
    if (filters.from) c.push(fwWhere('originNorm', '==', String(filters.from).trim().toLowerCase()));
    if (filters.to)   c.push(fwWhere('destNorm', '==', String(filters.to).trim().toLowerCase()));
    return c;
  }, [filters.from, filters.to]);

  const orderByField = filters.date ? 'departureAt' : 'createdAt';
  const orderDir = filters.date ? 'asc' : 'desc';
  const { rides, loading, eof, fetchPage, reset } = usePaginatedRides({ conditions, orderByField, orderDir, pageSize: 40 });
  ...
  const shownRides = useMemo(() => {
    ...
    const filtered = rides.filter(r => {
      ...
      // أخفِ إعلان الراكب إذا كان قد فُكّ قفله مسبقًا
      if (mapRole(r.role) === "passagerare" && r.bookingType === "contact_unlock") {
        // Keep visible if the current viewer just unlocked and chose to share info (persisted in sessionStorage)
        try {
          const raw = sessionStorage.getItem(`ride_unlock_shared_${r.id}`);
          if (raw) {
            const info = JSON.parse(raw);
            // show as visible once; do not filter out
          } else {
            return false;
          }
        } catch {
      ...
      // hide rides expired more than 7 days ago (UI-level)
      try {
        const ms = new Date(`${r.date}T${r.departureTime || '00:00'}`).getTime();
        const sevenDays = 7 * 24 * 60 * 60 * 1000;
        if (Number.isFinite(ms) && (Date.now() - ms) > sevenDays) return false;
      } catch {}
      return true;
    });
