...
export default function CompanyDetailSidebar({
  ...
  return (
    <div className="lg:sticky lg:top-24 space-y-3">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
          ...
          <div className="mb-3 text-center">
            <div className="flex items-baseline justify-center gap-2">
              {hasDiscount && discountedPrice && discountedPrice < lowestPrice ? (
                <>
                  <span className="text-2xl font-bold text-gray-900">{discountedPrice} kr</span>
                  <span className="text-base text-gray-400 line-through">{lowestPrice} kr</span>
                </>
              ) : (
                <span className="text-2xl font-bold text-gray-900">{lowestPrice} kr</span>
              )}
            </div>
            <p className="text-xs text-gray-500 mt-0.5">Från pris per tjänst</p>
          </div>
        )}
        ...
        <div className="space-y-2">
          <button
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-lg font-semibold shadow-sm hover:shadow-md transition-all"
          >
            Boka nu
          </button>
            ...
            (() => {
              ...
              return showFullPhone ? (
                <a
                  ...
                  className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-gray-300 text-gray-900 rounded-lg font-medium hover:bg-gray-50 hover:border-gray-400 transition text-sm"
                >
                  <HiPhone className="w-4 h-4" />
                  {displayPhone}
                </a>
              ) : (
                <div className="w-full flex flex-col items-center gap-1.5 px-4 py-2.5 bg-gray-50 border border-gray-200 text-gray-500 rounded-lg">
                  <div className="flex items-center gap-2">
                    <HiPhone className="w-4 h-4" />
                    <span className="font-medium text-sm">{displayPhone}</span>
                    <HiLockClosed className="w-3.5 h-3.5 text-amber-500" />
                  </div>
                  <p className="text-xs text-gray-400 text-center">Boka direkt via formuläret ovan</p>
                </div>
              )
            })()
          )}
      ...
      {/* Contact Card */}
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
        <h3 className="font-semibold text-gray-900 text-sm mb-3">Kontakt</h3>
        <div className="space-y-2.5">
          ...
          {company.phone && (
            <a
              href={`tel:${company.phone}`}
              className="flex items-center gap-2 text-xs text-gray-900 hover:text-blue-600 transition"
            >
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-gray-100 mt-2">
              ...
              {company.socialMedia.instagram && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1.5 text-xs text-gray-600 hover:text-pink-600 transition"
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
          ...
          {/* Organization Number */}
          {company.orgNumber && (
            <div className="flex items-center gap-2 text-xs text-gray-400 pt-2 border-t border-gray-100 mt-2">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
          ...
          <div className="space-y-3">
            ...
            {company.paymentMethods && company.paymentMethods.length > 0 && (
              <div>
                <p className="text-xs text-gray-400 mb-1.5">Betalningsmetoder</p>
                <div className="flex flex-wrap gap-1.5">
                  {company.paymentMethods.map((method) => (
                    <span
                      key={method}
                      className="inline-flex items-center px-2 py-0.5 bg-gray-50 text-gray-700 text-xs font-medium rounded-md border border-gray-200"
                    >
            ...
            {company.hasInsurance && (
              <div className="flex items-start gap-2">
                <HiShieldCheck className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                <div className="text-xs">
                  <p className="text-gray-900 font-semibold">Försäkrad</p>
                  {company.insuranceInfo && (
                    <p className="text-gray-500 mt-0.5">{company.insuranceInfo}</p>
                  )}
                </div>
              </div>
            )}
            ...
            {company.guarantee && (
              <div className="flex items-start gap-2">
                <HiBadgeCheck className="w-4 h-4 text-blue-500 flex-shrink-0 mt-0.5" />
                <div className="text-xs">
                  <p className="text-gray-900 font-semibold">Garanti</p>
                  <p className="text-gray-500 mt-0.5">{company.guarantee}</p>
                </div>
              </div>
            )}
          </div>
